"""Smoke-test HORS-LIGNE du pipeline RAG MultiLLM (sans appeler l'API).
...
DIM = 24
FAKE_MODEL = "selftest/Fake-Model-1B"
...
def _fake_llm_decision(group, label, description, candidates, examples, model):
    del group, label, description, examples, model
    mappings = []
...
def main() -> int:
    tmp = Path(tempfile.mkdtemp(prefix="rag_multillm_selftest_"))
    ...
    config.MULTI_LLM_MODELS = [FAKE_MODEL]
    config.TOGETHER_CHAT_MODEL = FAKE_MODEL
    ...
    generator._llm_decision = _fake_llm_decision
    ...
    out_dir = generate_mapping.run_mode("with_examples", FAKE_MODEL, limit=6)
