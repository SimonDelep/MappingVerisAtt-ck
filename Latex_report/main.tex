...
Nous avons choisi de mettre trois approches sont mises en concurrence~: \textbf{RAG} (Retrieval-Augmented
...
Ces correspondances sont produites et publiées par le CTID / MITRE Engenuity dans l'outil \emph{Mappings Explorer}~\cite{ctid_mappings}. Elles constituent dans notre travail une double référence : d'une part, elles définissent la cible que nos solutions d'intelligence artificielle cherchent à reproduire automatiquement ; d'autre part, elles servent de base d'évaluation pour mesurer la qualité des mappings générés. Le problème traité dans ce projet n'est donc pas seulement de produire des associations plausibles entre VERIS et ATT\&CK, mais bien de se rapprocher le plus possible d'un mapping expert déjà établi.

Toutefois, ce travail de correspondance ne porte pas sur l'ensemble du schéma VERIS, mais uniquement sur sept groupes dits ``comportementaux'', considérés comme les plus pertinents pour un rapprochement avec ATT\&CK. Ces capability groups couvrent principalement les actions menées lors d'un incident ainsi que certains impacts observables sur l'intégrité, la confidentialité, la disponibilité ou la chaîne de développement. Le Tableau~\\ref{tab:capability-groups} présente les volumes de référence associés à ces sept groupes pour la paire VERIS~1.4.1 / ATT\&CK~19.1.
